...
const regions: { value: RegionName, label: string }[] = [
  ...
  <div class="trivia-page">
    ...
    >
      ...
      <p class="text-base sm:text-lg text-gray-400 mb-2">
        A Pokémon will be shown — guess its type(s)!
      </p>
      <p class="text-xs sm:text-sm text-gray-500 mb-6">
        You have <strong class="text-white">30 seconds</strong> per question. Select all types that apply and confirm your answer.
      </p>
    ...
    >
      <UIcon
        ...
        class="text-5xl animate-spin text-primary mb-4"
      />
      ...
      <div class="sticky top-0 z-10 bg-default pb-2 pt-2 shrink-0">
        <div class="flex items-center justify-between mb-2">
          ...
          <UBadge
            ...
            size="lg"
          >
            Score: {{ score }} / {{ round - (answered ? 0 : 1) }}
          </UBadge>
        </div>
        <TriviaTimer
          v-if="!answered"
          :time-left="timeLeft"
          :max-time="TIMER_DURATION"
        />
      </div>

      <!-- Game Content: Side by Side -->
      <div class="flex flex-col md:flex-row gap-4 md:gap-10 items-center justify-center flex-1 min-h-0 mt-2 md:-mt-8 pb-4">
        ...
        <div class="flex flex-col items-center md:items-start">
          <p class="text-xs text-gray-400 mb-2">
            {{ answered ? 'Results:' : 'Select the type(s) of this Pokémon:' }}
          </p>
          <div class="options-grid">
            <TypeButton
              ...
              :is-correct-type="correctTypes.includes(t)"
              @click="toggleType"
            />
          </div>
